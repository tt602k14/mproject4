...
public class ProgramExerciseSessionActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Lưu ngay khi bắt đầu tập
        int totalSec = 0;
        if (exercises != null) for (Exercise ex : exercises) totalSec += ex.durationSeconds;
        int cal = exercises != null ? exercises.size() * 12 : 30;
        String pType = getIntent().getStringExtra("program_type");
        WorkoutSaver.save(this, "Chương trình " + (pType != null ? pType : "") + " ngày", Math.max(1, totalSec / 60), cal);
    }
    ...
    private void showCompletionDialog() {
        int totalSeconds = 0;
        if (exercises != null) for (Exercise ex : exercises) totalSeconds += ex.durationSeconds;
        int durationMinutes = Math.max(1, totalSeconds / 60);
        int calories = exercises != null ? exercises.size() * 12 : 30;
