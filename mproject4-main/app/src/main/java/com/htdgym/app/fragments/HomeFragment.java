...
public class HomeFragment extends Fragment {
    ...
    public void onResume() {
        super.onResume();
        // Delay nhỏ để đảm bảo WorkoutSaver background thread đã ghi xong DB
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(
                this::loadWorkoutStats, 600);
    }
    ...
    private void loadWorkoutStats() {
        ...
        new Thread(() -> {
            try {
                android.content.SharedPreferences prefs = ctx.getSharedPreferences("HTDGymPrefs", android.content.Context.MODE_PRIVATE);
                String userId = prefs.getString("userId", "guest");

                android.util.Log.d("HomeFragment", "loadWorkoutStats userId=" + userId);

                com.htdgym.app.database.GymDatabase db = com.htdgym.app.database.GymDatabase.getInstance(ctx);
                com.htdgym.app.models.WorkoutStats stats = db.workoutStatsDao().getStatsByUserId(userId);

                android.util.Log.d("HomeFragment", "stats=" + (stats == null ? "null" : "cal=" + stats.getTotalCalories() + " workouts=" + stats.getTotalWorkouts()));
                ...
                android.util.Log.e("HomeFragment", "loadWorkoutStats error", e);
                e.printStackTrace();
            }
        }).start();
    }
    ...
    private void loadProgramCards() {
        ...
        // Premium programs (60 and 90 days)
    ...
    private void addProgramCard(String title, String subtitle, String startDay, String endDay, String startDayVideoUrl, String endDayVideoUrl, String cardImageUrl) {
        addProgramCard(title, subtitle, startDay, endDay, startDayVideoUrl, endDayVideoUrl, cardImageUrl, false);
    }
    
    private void addProgramCard(String title, String subtitle, String startDay, String endDay, String startDayVideoUrl, String endDayVideoUrl, String cardImageUrl, boolean isPremium) {
        ...
        com.htdgym.app.utils.PremiumManager premiumManager = 
            com.htdgym.app.utils.PremiumManager.getInstance(requireContext());
        boolean isUserPremium = premiumManager.isPremiumUser();
        boolean isLocked = isPremium && !isUserPremium;
        
        // Create relative layout for lock icon positioning
        android.widget.RelativeLayout containerLayout = new android.widget.RelativeLayout(requireContext());
        ...
        startDayCard.setCardBackgroundColor(isLocked ? 0xFFCCCCCC : 0xFF6FCF97);
        ...
        startDayCard.setOnClickListener(v -> {
            if (isLocked) {
                showPremiumUpgradeDialog();
                return;
            }
            openYouTubeVideo(startDayVideoUrl);
        });
        ...
        endDayCard.setCardBackgroundColor(isLocked ? 0xFFCCCCCC : 0xFF6FCF97);
        ...
        endDayCard.setOnClickListener(v -> {
            if (isLocked) {
                showPremiumUpgradeDialog();
                return;
            }
            openYouTubeVideo(endDayVideoUrl);
        });
        ...
        tvTitle.setTextColor(isLocked ? 0xFF888888 : 0xFFFFFFFF);
        ...
        tvSubtitle.setTextColor(isLocked ? 0xFF888888 : 0xFFFFFFFF);
        ...
        // Add lock overlay + icon for premium content (card lớn)
        if (isLocked) {
            // Overlay tối phủ toàn bộ card
            android.view.View overlay = new android.view.View(requireContext());
            android.widget.RelativeLayout.LayoutParams overlayParams =
                new android.widget.RelativeLayout.LayoutParams(
                    android.widget.RelativeLayout.LayoutParams.MATCH_PARENT,
                    android.widget.RelativeLayout.LayoutParams.MATCH_PARENT);
            overlay.setLayoutParams(overlayParams);
            overlay.setBackgroundColor(0xBB000000);
            containerLayout.addView(overlay);

            // Lock icon to ở giữa — set id trước khi dùng
            int lockIconId = android.view.View.generateViewId();
            ImageView lockIcon = new ImageView(requireContext());
            lockIcon.setId(lockIconId);
            int iconSize = (int) (56 * getResources().getDisplayMetrics().density);
            android.widget.RelativeLayout.LayoutParams lockParams =
                new android.widget.RelativeLayout.LayoutParams(iconSize, iconSize);
            lockParams.addRule(android.widget.RelativeLayout.CENTER_HORIZONTAL);
            lockParams.addRule(android.widget.RelativeLayout.CENTER_VERTICAL);
            lockIcon.setLayoutParams(lockParams);
            lockIcon.setImageResource(R.drawable.ic_lock_premium);
            lockIcon.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
            containerLayout.addView(lockIcon);

            // Text "Nâng cấp Premium" bên dưới icon
            TextView tvLockLabel = new TextView(requireContext());
            android.widget.RelativeLayout.LayoutParams labelParams =
                new android.widget.RelativeLayout.LayoutParams(
                    android.widget.RelativeLayout.LayoutParams.WRAP_CONTENT,
                    android.widget.RelativeLayout.LayoutParams.WRAP_CONTENT);
            labelParams.addRule(android.widget.RelativeLayout.CENTER_HORIZONTAL);
            labelParams.addRule(android.widget.RelativeLayout.BELOW, lockIconId);
            labelParams.setMargins(0, (int) (8 * getResources().getDisplayMetrics().density), 0, 0);
            tvLockLabel.setLayoutParams(labelParams);
            tvLockLabel.setText("👑 Nâng cấp Premium");
            tvLockLabel.setTextColor(0xFFFFD700);
            tvLockLabel.setTextSize(13);
            tvLockLabel.setTypeface(null, android.graphics.Typeface.BOLD);
            tvLockLabel.setGravity(android.view.Gravity.CENTER);
            containerLayout.addView(tvLockLabel);
        }
    ...
    private void loadBuildMusclePrograms() {
        ...
        // Premium exercises (all locked for non-premium users)
        addProgramItem("Tập Tay Cơ Bản 👑", 0, "https://youtu.be/hRJ6tR5-if0", true); 
        addProgramItem("Push-ups Cho Người Mới 👑", 0, "https://youtu.be/-R5sH2iG9Gw", true);
        addProgramItem("Bộ Ngực Vạm Vỡ Trong 45 Ngày 👑", 0, "https://youtu.be/XtoO9YwNEqA", true);
        addProgramItem("Đôi Chân Mạnh Mẽ Trong 45 Ngày 👑", 0, "https://youtu.be/7SLbUk-qTTM", true);
        addProgramItem("Bài Tập Lưng Và Vai Nâng Cao 👑", 0, "https://youtu.be/JZQA08SlJnM", true);
        addProgramItem("Tập Luyện Toàn Thân Pro 👑", 0, "https://youtu.be/Y8VflnViz78", true);
    }
    ...
    private void loadBurnFatPrograms() {
        ...
        // Premium exercises (all locked for non-premium users)
        addProgramItem("Giảm Mỡ Bụng Cơ Bản 👑", 0, "https://youtu.be/GU5_SdDdc8o", true);
        addProgramItem("Cardio Cho Người Mới 👑", 0, "https://youtu.be/c4DAnQ6DtF8", true);
        addProgramItem("Giảm Mỡ Toàn Thân - Đạch Chế Nguyễn Mỗi 👑", 0, "https://youtu.be/1BZM2Vre5oc", true);
        addProgramItem("Đốt Mỡ Thần Tốc - Trang Cấp 👑", 0, "https://youtu.be/3XDriUn0udo", true);
        addProgramItem("HIIT Giảm Cân Nhanh 👑", 0, "https://youtu.be/ZpUre6uZQ7E", true);
        addProgramItem("Lịch Tập Giảm Cân 👑", 0, "https://youtu.be/ml6cT4AZdqI", true);
    }



    private void addProgramItem(String title, int progress, String videoUrl) {
        addProgramItem(title, progress, videoUrl, false);
    }
    
    private void addProgramItem(String title, int progress, String videoUrl, boolean isPremium) {
        ...
        TextView tvTitle = new TextView(requireContext());
        tvTitle.setText(title);
        tvTitle.setTextColor(isLocked ? 0xFF888888 : 0xFF1A1A1A);
        tvTitle.setTextSize(15);
        tvTitle.setTypeface(null, android.graphics.Typeface.BOLD);
        
        // Add premium badge to title if premium
        if (isPremium) {
            String premiumTitle = title + " 👑";
            tvTitle.setText(premiumTitle);
        }
        ...
        progressBar.setProgressTintList(android.content.res.ColorStateList.valueOf(
                isLocked ? 0xFFCCCCCC : 0xFF6FCF97));
        ...
        tvArrow.setText("›");
        tvArrow.setTextColor(isLocked ? 0xFFCCCCCC : 0xFFC0C0C0);
        ...
        // Add lock overlay + icon for premium content (card danh sách)
        if (isLocked) {
            // Overlay tối phủ toàn card
            android.view.View overlay = new android.view.View(requireContext());
            android.widget.RelativeLayout.LayoutParams overlayParams =
                new android.widget.RelativeLayout.LayoutParams(
                    android.widget.RelativeLayout.LayoutParams.MATCH_PARENT,
                    android.widget.RelativeLayout.LayoutParams.MATCH_PARENT);
            overlay.setLayoutParams(overlayParams);
            overlay.setBackgroundColor(0xAA000000); // 67% đen
            containerLayout.addView(overlay);

            // Lock icon ở giữa bên phải (cạnh arrow)
            ImageView lockIcon = new ImageView(requireContext());
            int iconSize = (int) (36 * getResources().getDisplayMetrics().density);
            android.widget.RelativeLayout.LayoutParams lockParams =
                new android.widget.RelativeLayout.LayoutParams(iconSize, iconSize);
            lockParams.addRule(android.widget.RelativeLayout.CENTER_VERTICAL);
            lockParams.addRule(android.widget.RelativeLayout.ALIGN_PARENT_END);
            lockParams.setMargins(0, 0, (int) (12 * getResources().getDisplayMetrics().density), 0);
            lockIcon.setLayoutParams(lockParams);
            lockIcon.setImageResource(R.drawable.ic_lock_premium);
            lockIcon.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
            containerLayout.addView(lockIcon);
        ...
        programCard.addView(containerLayout);
    ...
    private void checkPremiumStatus() {
        ...
        // Show premium status in a toast (you can customize this)
        if (isPremium) {
            Toast.makeText(requireContext(), "🎉 Chào mừng thành viên Premium!", Toast.LENGTH_SHORT).show();
        } else {
            // Optionally show a subtle premium promotion
            // Toast.makeText(requireContext(), "💎 Nâng cấp Premium để truy cập đầy đủ tính năng", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1001 && resultCode == getActivity().RESULT_OK) {
            // Premium was activated, refresh the UI
            if (data != null && data.getBooleanExtra("premium_activated", false)) {
                // Show success message
                new androidx.appcompat.app.AlertDialog.Builder(requireContext())
                    .setTitle("🎉 Premium đã kích hoạt!")
                    .setMessage("Chúc mừng! Bạn đã trở thành thành viên Premium.\n\n" +
                        "Tất cả nội dung đã được mở khóa. Hãy khám phá ngay!")
                    .setPositiveButton("Khám phá ngay", (dialog, which) -> {
    ...
    private void refreshCurrentTab() {
        // Refresh the current tab content to show unlocked premium content
        if (currentTab.equals("build_muscle")) {
            loadBuildMusclePrograms();
        } else if (currentTab.equals("burn_fat")) {
            loadBurnFatPrograms();
        }
        
        // Also refresh program cards
        loadProgramCards();
